...
define('SMTP_USER', 'nexusclub.noreply@gmail.com');
define('SMTP_PASS', 'akrb crgl owld fcdp');
...
function getToken()
{
    ...
    if (function_exists('apache_request_headers')) {
        ...
        if (!empty($h['Authorization']))
            return trim(str_replace('Bearer', '', $h['Authorization']));
        if (!empty($h['authorization']))
            return trim(str_replace('Bearer', '', $h['authorization']));
        if (!empty($h['X-Auth-Token']))
            return trim($h['X-Auth-Token']);
        if (!empty($h['x-auth-token']))
            return trim($h['x-auth-token']);
    }
...
function requireAuth()
{
    $token = getToken();
    if (!$token)
        respondError('No token — please sign in first', 401);
    $user = jwtDecode($token);
    if (!$user)
        respondError('Session expired — please sign in again', 401);
    return $user;
}
